...
Atrue = [sysd.A - sysd.B*Kopt, sysd.B*Kopt;
...
[H, f, Actrue, Axtrue, b1, lb, ub, options] = MPC_vars(Atrue, Btrue, C, Kopt, R, p, main_bounds, maxF);
%%
...
for k = 1 : Time_out/Ts - 1
    ...
    b = b1 + Axtrue*X;
    ck = quadprog(H, f, -Actrue, -b, [], [], lb, ub, [], options);
    if isempty(ck)
        c = 0;
    else
        c = ck(1);
